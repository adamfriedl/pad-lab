...
# Phase 1: APIs + Artifact Registry + secret shell (image must exist before Job).
TF_FOUNDATION=(
...
echo "==> terraform init / apply (foundation — AR + secret, no Cloud Run yet)..."
(
  cd "$TF_DIR"
  terraform init -input=false -backend-config="bucket=${TFSTATE_BUCKET}"
  terraform apply -input=false -auto-approve "${TF_FOUNDATION[@]}"
)
...
echo "==> terraform apply (datasets, Cloud Run Job, Scheduler, image trigger, monitoring)..."
echo "    Note: Cloud Build GitHub trigger needs a one-time repo connection (region ${REGION})."
(
  cd "$TF_DIR"
...
echo "    Rebuild image:         ./scripts/build_image.sh"
echo "    Viz data URL (Pages):  set GitHub Actions variable VITE_DATA_BASE_URL to viz_data_base_url output"
