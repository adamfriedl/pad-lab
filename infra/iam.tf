...
# Scheduler SA: start Cloud Build (build image + run job)
resource "google_project_iam_member" "scheduler_cloudbuild_editor" {
  project = var.project_id
  role    = "roles/cloudbuild.builds.editor"
  member  = "serviceAccount:${google_service_account.scheduler.email}"
}

# Cloud Build SA: execute the Cloud Run Job after pushing a new image
resource "google_cloud_run_v2_job_iam_member" "cloudbuild_job_runner" {
  project  = var.project_id
  location = var.region
